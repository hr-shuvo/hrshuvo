...
const contacts = [
  ...
  },
  {
    label: "Facebook",
    handle: "shuvo",
    href: "https://www.facebook.com/raskolnikov.shuvo/",
    icon: (
      <svg width={14} height={14} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
  ...
  },
  {
    label: "Instagram",
    handle: "hrshuv0",
    href: "https://www.instagram.com/hrshuv0/",
    icon: (
      <svg width={14} height={14} viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        ...
        <div className="relative overflow-hidden mt-24 border-t border-[var(--border)] pt-14 lg:mt-32">
          ...
          <div>
            {journeyItems.map((item) => (
              <div
                key={item.year}
                className="group relative grid border-b border-[var(--border)] py-7 gap-3 lg:grid-cols-[6rem_1fr_auto] lg:gap-10 lg:items-start hover:bg-[var(--surface-alt)] transition-colors duration-200 -mx-2 px-2"
              >
                <span
                  className="absolute left-0 top-0 bottom-0 w-[2px] bg-transparent group-hover:bg-[var(--color-accent)] transition-colors duration-200"
                  aria-hidden="true"
                />
                <p
                  className="font-serif select-none tabular-nums font-semibold text-[var(--muted-color)] group-hover:text-[var(--color-accent)] transition-colors duration-200"
                  style={{
                ...
                <div className="flex flex-wrap gap-1.5 lg:justify-end lg:max-w-[14rem]">
                  {item.tags.slice(0, 2).map((tag) => (
                    <span
                      key={tag}
                      className="text-xs border border-[var(--border)] px-2 py-0.5 text-[var(--muted-color)] group-hover:text-[var(--color-accent)] group-hover:border-[var(--color-accent)]/50 transition-colors duration-200"
                    >
                      {tag}
                    </span>
                  ))}
        ...
        <div className="relative overflow-hidden mt-24 border-t border-[var(--border)] pt-14 lg:mt-32">
          ...
          <div className="flex flex-wrap gap-3">
            {interests.map((item) => (
              <span
                key={item}
                className="text-sm border border-[var(--border)] px-4 py-2 text-[var(--muted-color)] hover:text-[var(--color-accent)] hover:border-[var(--color-accent)]/50 transition-colors duration-200"
              >
                {item}
              </span>
            ))}
          ...
          <div className="flex flex-col max-w-2xl">
            {contacts.map((c) => (
              <a
                ...
                className="group relative flex items-center gap-6 py-7 border-b border-[var(--border)] hover:bg-[var(--surface-alt)] hover:border-[var(--color-accent)]/40 transition-colors duration-200 -mx-4 px-4"
              >
                <span
                  className="absolute left-0 top-0 bottom-0 w-[3px] bg-transparent group-hover:bg-[var(--color-accent)] transition-colors duration-200"
                  aria-hidden="true"
                />
                <span className="w-5 h-5 flex-shrink-0 text-[var(--muted-color)] group-hover:text-[var(--color-accent)] transition-colors duration-200 [&_svg]:w-full [&_svg]:h-full">
                  {c.icon}
                </span>
                <span className="text-xs tracking-[0.1em] uppercase text-[var(--muted-color)] w-24 flex-shrink-0">{c.label}</span>
                <span className="text-base sm:text-lg font-medium text-[var(--foreground)] group-hover:text-[var(--color-accent)] transition-colors duration-200 flex-1 truncate">
                  {c.handle}
                </span>
                <span className="flex-shrink-0 text-sm text-[var(--muted-color)] group-hover:text-[var(--color-accent)] transition-colors duration-200">↗</span>
              </a>
            ))}
          </div>
        </div>
